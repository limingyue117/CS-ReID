...
class GraphSampler_BAS_LTCC(Sampler):
    ...
    def graph_index(self):
        ...
        for i in range(self.num_identities):
            ...
            index = []
            pid_numn = defaultdict(list)
            mn = 0 
            while(mn<self.batch_size):
                for ij in id_index:  
                    pid = self.pids[ij]
                    for ip in self.pid_num[pid]:
                        pid_numn[ij].append(ip)
                        mn+=1
                        if mn == self.batch_size:
                            break
                    if mn == self.batch_size:
                            break
            for j in id_index:                       #每类取k个样本
                ...
                pid_n = len(pid_numn[j])
                random.shuffle(self.pid_num[pid])
