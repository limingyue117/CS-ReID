...
class PRCC_SKE(object):
    ...
    def _process_dir_train(self, dir_path, ske_path):
        ...
        for pdir in pdirs:
            pid = int(osp.basename(pdir))
            sketch_pid = osp.join(ske_path, osp.basename(pdir))
            img_dirs = glob.glob(osp.join(pdir, '*.jpg'))
            for img_dir in img_dirs:
                cam = osp.basename(img_dir)[0] # 'A' or 'B' or 'C'
                label = pid2label[pid]
                camid = cam2label[cam]
                sketch_dir = osp.join(sketch_pid, osp.basename(img_dir))
                if cam in ['A', 'B']:
                    clothes_id = clothes2label[osp.basename(pdir)]
                else:
                    clothes_id = clothes2label[osp.basename(pdir)+osp.basename(img_dir)[0]]
                dataset.append((img_dir, sketch_dir, label, camid, clothes_id))
    ...
    def _process_dir_test(self, test_path, test_ske_path):
        ...
        for cam in ['A', 'B', 'C']:
            ...
            for pdir in pdirs:
                pid = int(osp.basename(pdir))
                sketch_pid = osp.join(test_ske_path, cam, osp.basename(pdir))
                img_dirs = glob.glob(osp.join(pdir, '*.jpg'))
                for img_dir in img_dirs:
                    # pid = pid2label[pid]
                    camid = cam2label[cam]
                    ...
                    if cam == 'A':
                        ...
                        gallery_dataset.append((img_dir, sketch_dir, pid, camid, clothes_id))
                    elif cam == 'B':
                        clothes_id = pid2label[pid] * 2
                        query_dataset_same_clothes.append((img_dir, sketch_dir, pid, camid, clothes_id))
                    else:
                        clothes_id = pid2label[pid] * 2 + 1
                        query_dataset_diff_clothes.append((img_dir, sketch_dir, pid, camid, clothes_id))

        pid2imgidx = {}
        for idx, (img_dir, sketch_dir, pid, camid, clothes_id) in enumerate(gallery_dataset):
