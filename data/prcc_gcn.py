...
class PRCC_GCN(BaseDataset):
    ...
    --------------------------------------
      ...
      gallery        |    71     |     3384
    """
    rgb_dir = 'prcc_gcn/rgb'
    msk_dir = 'prcc_gcn/mask_6'
    cam2label = {'A': 0, 'B': 1, 'C': 2}

    def __init__(self, root='data', verbose=True, mask='mask', **kwargs):
        super(PRCC_GCN, self).__init__()
        self.root = root
        self.dataset_dir = osp.join(self.root, self.rgb_dir)
        self.train_dir = osp.join(self.dataset_dir, 'train')
        self.mask_dir = osp.join(self.root, self.msk_dir)

        self.query_c = osp.join(self.dataset_dir, 'test', 'C')
        self.query_b = osp.join(self.dataset_dir, 'test', 'B')
        self.gallery_dir = osp.join(self.dataset_dir, 'test', 'A')

        self._check_before_run()

        self.pid2label = self.get_pid2label(self.train_dir)
        self.train = self._process_dir(self.train_dir, os.path.join(self.mask_dir, 'train'), pid2label=self.pid2label, relabel=True)          # 13081
        self.train_ca = self._process_train(self.train_dir, os.path.join(self.mask_dir, 'train'), select=['A', 'C'], pid2label=self.pid2label, relabel=True)
        self.train_cb = self._process_train(self.train_dir, os.path.join(self.mask_dir, 'train'), select=['A', 'B'], pid2label=self.pid2label, relabel=True)
        self.query_c = self._process_test(self.query_c, os.path.join(self.mask_dir, 'test', 'C'), cid='C', relabel=False)       # 484
        self.query_b = self._process_test(self.query_b, os.path.join(self.mask_dir, 'test', 'B'), cid='B', relabel=False)
        self.gallery = self._process_test(self.gallery_dir, os.path.join(self.mask_dir, 'test', 'A'), cid='A', relabel=False)
    ...
    def _check_before_run(self):
        ...
        if not osp.exists(self.query_c):
            raise RuntimeError("'{}' is not available".format(self.query_c))
        if not osp.exists(self.query_b):
            raise RuntimeError("'{}' is not available".format(self.query_b))
        if not osp.exists(self.gallery_dir):
            raise RuntimeError("'{}' is not available".format(self.gallery_dir))

    def _process_dir(self, dir_path, mask_dir, pid2label=None, relabel=False):
        persons = os.listdir(dir_path)
        dataset = []
        for pid_s in persons:
            ...
            for file in files:
                ...
                name = file.split('.')[0] + '.npy'
                msk_path = os.path.join(mask_dir, pid_s, name)
                if not os.path.exists(msk_path):
                    continue
                dataset.append((img_path, pid, cid, msk_path))
        return dataset

    def _process_train(self, dir_path, mask_dir, select=['A', 'C'], pid2label=None, relabel=False):
        persons = os.listdir(dir_path)
        dataset = []
        for pid_s in persons:
            path_p = os.path.join(dir_path, pid_s)
    ...
    def _process_test(self, dir_path, mask_dir, cid='C', pid2label=None, relabel=False):
        cid = self.cam2label[cid]
        persons = os.listdir(dir_path)
        dataset = []
        for pid_s in persons:
